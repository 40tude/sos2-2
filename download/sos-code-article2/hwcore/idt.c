...
sos_ret_t sos_idt_setup() {
  ...
  for (i = 0; i < SOS_IDTE_NUM; i++) {
    ...
    sos_idt_set_wrapper(i, (sos_vaddr_t)NULL, 0 /* Don't care */);
  }
...
sos_ret_t sos_idt_set_wrapper(int index, sos_vaddr_t handler_address, int lowest_priviledge /* 0..3 */) {
...
sos_ret_t sos_idt_get_wrapper(int index, sos_vaddr_t *handler_address, int *lowest_priviledge) {
  if ((index < 0) || (index >= SOS_IDTE_NUM))
    return -SOS_EINVAL;

  if (handler_address != NULL)
    *handler_address = idt[index].offset_low | (idt[index].offset_high << 16);
