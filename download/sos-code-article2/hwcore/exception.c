...
sos_ret_t sos_install_dbl_fault_exceptions(void)
{
  /* We inidicate that the double fault exception handler is defined,
     ...
     functions below */
  return sos_idt_set_wrapper(SOS_EXCEPT_BASE + SOS_EXCEPT_DOUBLE_FAULT,
				    ...
				    sos_exception_handler_t routine)
{
  ...
  if (routine != NULL)
    retval
      = sos_idt_set_wrapper(SOS_EXCEPT_BASE + exception_number,
